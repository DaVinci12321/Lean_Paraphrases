#!/usr/bin/env python3
"""
autoformalize_claude.py -- stage 2 of the Lean Paraphrases extension.

Autoformalizes every paraphrase produced by generate_paraphrase_dataset.py into
a Lean 4 statement with `sorry` as the proof, using Anthropic's Claude Opus 5.

This script and its sibling (autoformalize_gpt.py) are identical apart from
...
Output (in --outdir, default ./formalized/claude)
----------------------------------------------------------
...
Usage
-----
    python autoformalize_claude.py                        # formalize the whole dataset
    python autoformalize_claude.py --limit 10             # pilot on the first 10 problems
    python autoformalize_claude.py --resume               # continue an interrupted run
    python autoformalize_claude.py --dry-run --outdir /tmp/x
"""
...
# ---------------------------------------------------------------------------
MODEL_LABEL = "claude"
MODEL_NAME = ANTHROPIC_MODEL          # "claude-opus-5"
MODEL_DISPLAY = "Anthropic's Claude Opus 5"
REQUIRED_ENV = "ANTHROPIC_API_KEY"


def call_model(clients: Clients, prompt: str) -> str:
    return clients.anthropic_text(prompt, model=MODEL_NAME)
# ---------------------------------------------------------------------------
